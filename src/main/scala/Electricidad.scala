   ...
   Logger.getLogger("org").setLevel(Level.WARN)
    ...
    for (a <- anyo) {
     ...
     /* pw.write("Day consumption")
      ...
      pw.write(consumoDia.collect().toList.toString())
      ...
      pw.write(consumo0.length.toString()+ " Values zero or less than zero\n")
    ...
    pw.write((consumoPorHora.collect().toList.toString()))
    ...
    pw.write(consumoMedioPorHoraRegistros.collect().toList.toString())

    pw.write("\nMonth consumption")

    val consumoMes = data.map(line => (line._4.toString, line._2)).reduceByKey(_ + _).sortBy(_._1)

    pw.write(consumoMes.collect().toList.toString())
    ...
    pw.write(ctotal.toString)

    pw.write("\nYear consumption")

    pw.write(consumoMes.map(line => (line._2)).reduce(_ + _).toString())
    ...
    pw.write(chm.collect().toList.toString())
    ...
    pw.write(cmhm.collect().toList.toString())
    ...
    pw.write("Skewness: " + valuesStatistics.getSkewness()) //Oblique - Symmetry Coeficient ( Normal value between : +- 0.5)
    ...
    pw.write("Variation Coeficient: " + variationCoeficient)
    ...
    pw.write("Variation Coeficient: " + variationCoeficient2)
    ...
    pw.write("Variation Coeficient: " + variationCoeficient3)
